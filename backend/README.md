...
│   ├── core/db.py         # shared psycopg connection pool
│   ├── features/
...
├── scripts/               # CLI chat + memory inspection scripts (not app code)
└── initdb/                # schema SQL, run by hand against Postgres
```

(`auth/`, `users/`, `features/scripts/`, `memory/graph/` etc. exist as empty placeholders.)
...
MLflow tracing must be running first -- despite the commented-out tracing block in `app/orchestration/llm_client.py`,
...
uv run python -m scripts.test_true_memory      # needs Postgres + AICREDITS_API_KEY; drives the real graph
```
